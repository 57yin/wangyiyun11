...
def fetch_comments_detailed(song_id, current_playlist_name):
    ...
    try:
        # 步骤1：获取总评论数，计算目标普通评论数（纯比例，仅保留下限20条）
        total_comment_count = get_total_comments_count(song_id, headers)
        
        # 关键：判断是否是“新歌榜”，动态设置比例
        if current_playlist_name == "新歌榜":
            current_ratio = 0.005  # 新歌榜用1%比例
        else:
            current_ratio = TARGET_NORMAL_COMMENT_RATIO  # 其他歌单用默认比例
        ...
        page_url = f'http://music.163.com/api/v1/resource/comments/R_SO_4_{song_id}?limit={COMMENTS_PER_REQUEST}&offset={offset}'
        try:
        ...
        # 步骤3：抓取普通评论（按目标数量动态采集，直到满足比例）
        collected_normal_comments = 0
        page = 1  # 从第2页开始抓取普通评论（第1页已抓热评）
        while collected_normal_comments < target_normal_comments:
            offset = page * COMMENTS_PER_REQUEST
            # 计算当前页需要获取的数量（避免最后一页超出目标）
            ...
            try:
                ...
                if page_data.get('code') != 200:
                    print(f"    - 普通评论第{page+1}页请求失败，错误码: {page_data.get('code')}，停止抓取")
                    break

                normal_comments = page_data.get('comments', [])
                if not normal_comments:
                    print(f"    - 普通评论第{page+1}页无数据，停止抓取")
                    break
                ...
                print(f"    - 普通评论第{page+1}页请求异常: {str(e)}，停止抓取")
                break
        
        # 去重：使用commentId，确保每条评论唯一
        unique_comments_dict = {}
        for comment in all_raw_comments:
            comment_id = comment.get('commentId')
            if comment_id:
                unique_comments_dict[comment_id] = comment
        
        # 格式化评论（不截断，保留所有采集到的有效评论）
        final_comments = [_format_comment(c) for c in unique_comments_dict.values()]
        # 按点赞数降序排序（方便后续查看，不影响采集数量）
        final_comments = sorted(final_comments, key=lambda x: x["praise"], reverse=True)
        
        if not final_comments:
            print(f"  - 歌曲 ID {song_id} 未获取到任何有效评论,将返回空数据")
            return pd.DataFrame(), {}
        else:
            #正常处理有评论情况
            pass
...
def save_song_data_for_playlist(song_info, comments_df, summary, playlist_name):
    ...
    dataset_row = {
        ...
        '高频字眼': summary.get('high_freq_words', '').replace(",", "，"),
        '最高点赞量': summary.get('max_praise', 0),
        '平均点赞量': summary.get('avg_praise', 0),
        'Top1评论预览': summary.get('top_comment', '').replace(",", "，"),
    ...
    if not comments_df.empty:
        ...
        # 评论为空时，打印提示，不保存详细评论文件
        print(f"  - 歌曲 {song_info['name']} 评论为空，跳过详细评论保存")
...
def start_daily_schedule():
    ...
    print(f"\n定时任务已启动！每日 {DAILY_UPDATE_TIME} 自动执行评论抓取（纯百分比采集）")
...
if __name__ == "__main__":
    ...
    # 方式3：仅启动每日定时任务（适合服务器部署）
>>>>>>> 6013d9562b4a3b1f0ed7952deffda8de1afcc6f3
    # start_daily_schedule()